import "dotenv/config";
import { drizzle } from "drizzle-orm/node-postgres";
import { pgTable, text, timestamp, uuid } from "drizzle-orm/pg-core";
import { sql, inArray } from "drizzle-orm";
import pg from "pg";

const { Pool } = pg;

const prodConnStr = "postgresql://neondb_owner:npg_eKLtUT1Ou7dp@ep-misty-credit-az0zobat-pooler.c-3.ap-southeast-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require";

const pool = new Pool({
  connectionString: prodConnStr,
  ssl: { rejectUnauthorized: false }
});

const db = drizzle(pool);

async function runProdAudit() {
  console.log("=================================");
  console.log("STEP 1: VERIFY NEON DATABASE CONNECTION");
  console.log("=================================");
  
  let connInfo: any = {};
  try {
    const connRes = await db.execute(sql`
      SELECT 
        current_database() as current_db,
        current_schema() as current_schema,
        inet_server_addr() as server_ip,
        inet_server_port() as server_port,
        version() as pg_version;
    `);
    connInfo = connRes.rows[0];
    console.log("Neon Production Active Connection Info:", connInfo);
  } catch (err: any) {
    console.error("Error querying conn info on Neon:", err.message);
  }

  const url = new URL(prodConnStr);
  const host = url.hostname;
  const dbName = url.pathname.replace("/", "");
  const ssl = url.searchParams.get("sslmode") || "Not set";
  let branch = "Unknown";
  const match = host.match(/^([^-]+(-[^-]+)*)-pooler\./) || host.match(/^([^-]+(-[^-]+)*)\./);
  if (match) branch = match[1];

  console.log("Parsed Connection Details:", {
    host,
    dbName,
    sslConfig: ssl,
    neonBranchEndpoint: branch,
  });

  console.log("\n=================================");
  console.log("STEP 2: VERIFY DATABASE STRUCTURE");
  console.log("=================================");
  
  const tablesRes = await db.execute(sql`
    SELECT table_name 
    FROM information_schema.tables 
    WHERE table_schema = 'public';
  `);
  const existingTables = tablesRes.rows.map((r: any) => r.table_name);
  console.log("Existing Tables in public schema (Neon):", existingTables);

  const targetTables = ['users', 'account', 'session', 'verification', 'tenant', 'tenants', 'roles', 'permissions', 'user_roles'];
  const tableCheckStatus: Record<string, boolean> = {};
  targetTables.forEach(t => {
    tableCheckStatus[t] = existingTables.includes(t);
  });
  console.log("Target Table Verification Status:", tableCheckStatus);

  console.log("\n=================================");
  console.log("STEP 3: VERIFY MIGRATIONS");
  console.log("=================================");
  
  try {
    const migRes = await db.execute(sql`SELECT * FROM __drizzle_migrations ORDER BY created_at ASC;`);
    console.log("Executed Migrations count:", migRes.rows.length);
    console.log("Executed Migrations details:", migRes.rows);
  } catch (e: any) {
    console.log("No __drizzle_migrations table found in Neon DB.");
  }

  console.log("\n=================================");
  console.log("STEP 4: VERIFY AUTHENTICATION DATA");
  console.log("=================================");
  
  const getCount = async (tableName: string) => {
    if (!existingTables.includes(tableName)) return "TABLE NOT FOUND";
    try {
      const res = await db.execute(sql.raw(`SELECT COUNT(*) as cnt FROM "${tableName}"`));
      return (res.rows[0] as any).cnt;
    } catch (err: any) {
      return `Error: ${err.message}`;
    }
  };

  const counts: Record<string, any> = {};
  for (const t of ['users', 'account', 'session', 'verification', 'tenants', 'roles', 'permissions', 'user_roles']) {
    counts[t] = await getCount(t);
  }
  console.log("Table Row Counts (Neon):", counts);

  console.log("\n=================================");
  console.log("STEP 5: VERIFY DEFAULT USERS");
  console.log("=================================");
  
  const targetEmails = [
    'admin.it@sekolah1.sch.id',
    'kepsek@sekolah1.sch.id',
    'guru.budi@sekolah1.sch.id',
    'siswa.putra@sekolah1.sch.id'
  ];

  let foundUsers: any[] = [];
  if (existingTables.includes('users')) {
    const res = await db.execute(sql`SELECT id, email, name, role FROM users WHERE email IN ('admin.it@sekolah1.sch.id', 'kepsek@sekolah1.sch.id', 'guru.budi@sekolah1.sch.id', 'siswa.putra@sekolah1.sch.id')`);
    foundUsers = res.rows;
  }
  console.log("Found Target Users Count (Neon):", foundUsers.length);
  console.log("Found Target Users (Neon):", foundUsers);

  console.log("\n=================================");
  console.log("STEP 6: VERIFY ACCOUNT LINK");
  console.log("=================================");
  
  let accountsData: any[] = [];
  if (existingTables.includes('account')) {
    const res = await db.execute(sql`SELECT id, "account_id", "provider_id", "user_id", password FROM account`);
    accountsData = res.rows;
    console.log("Total accounts in 'account' table (Neon):", accountsData.length);
  }

  const userAccMapping = foundUsers.map(u => {
    const acc = accountsData.filter(a => a.user_id === u.id);
    return {
      userId: u.id,
      email: u.email,
      hasAccount: acc.length > 0,
      accountProviders: acc.map(a => a.provider_id),
      accountDetails: acc.map(a => ({ id: a.id, accountId: a.account_id, passwordLength: a.password ? a.password.length : 0 }))
    };
  });
  console.log("User-Account Mapping (Neon):", JSON.stringify(userAccMapping, null, 2));

  if (existingTables.includes('account') && existingTables.includes('users')) {
    const orphans = await db.execute(sql`
      SELECT a.id, a.user_id, a.provider_id 
      FROM account a 
      LEFT JOIN users u ON a.user_id = u.id 
      WHERE u.id IS NULL;
    `);
    console.log("Orphan accounts count (Neon):", orphans.rows.length);
  }

  console.log("\n=================================");
  console.log("STEP 7: VERIFY PASSWORD HASH");
  console.log("=================================");
  
  foundUsers.forEach(u => {
    const accs = accountsData.filter(a => a.user_id === u.id);
    if (accs.length === 0) {
      console.log(`User ${u.email}: NO ACCOUNT RECORD`);
    }
    accs.forEach(a => {
      let passStatus = "No password field / empty";
      let hashAlgo = "Unknown";
      if (a.password) {
        const pass = a.password;
        if (pass.startsWith("$2a$") || pass.startsWith("$2b$") || pass.startsWith("$2y$")) {
          passStatus = "Properly hashed";
          hashAlgo = "bcrypt";
        } else if (pass.startsWith("$argon2id$") || pass.startsWith("$argon2i$")) {
          passStatus = "Properly hashed";
          hashAlgo = "argon2";
        } else if (pass.startsWith("pbkdf2$") || pass.startsWith("$scrypt$")) {
          passStatus = "Properly hashed";
          hashAlgo = "pbkdf2/scrypt";
        } else if (pass.startsWith("scrypt$") || pass.includes(":")) {
          passStatus = "Properly hashed";
          hashAlgo = "scrypt (Better Auth default)";
        } else if (pass.length === 64 || pass.length === 32) {
          passStatus = "Possible raw hash (SHA256/MD5)";
        } else if (pass.length < 20) {
          passStatus = "PLAIN TEXT OR INSECURE";
        } else {
          passStatus = `Hashed format (length=${pass.length})`;
          hashAlgo = "scrypt / Better Auth Hash";
        }
      }
      console.log(`User ${u.email}: status="${passStatus}", algo="${hashAlgo}", samplePrefix="${a.password ? a.password.substring(0, 20) : 'N/A'}"`);
    });
  });

  await pool.end();
  process.exit(0);
}

runProdAudit().catch(err => {
  console.error("Prod Audit failed:", err);
  process.exit(1);
});
